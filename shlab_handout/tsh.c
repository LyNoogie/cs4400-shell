...
void eval(char *cmdline) 
{
  ...
  /* The parent must use sigprocmask to block SIGCHLD signals before it forks the child,
     and then unblock these signals, again using sigprocmask, after it adds the child to
     the job list by calling addjob. Since children inherit the blocked vectors of their
     parents, the child must be sure to then unblock SIGCHLD signals before it execs the
     new program.

     The parent needs to block the SIGCHLD signals in this way in order to avoid the race
     condition where the child is reaped by sigchld handler (and thus removed from the
     jobs list) before the parent calls addjob. Depending on your implementation, you may
     also need to block SIGINT and SIGTSTP similarly.
  */
  sigset_t blocked;
  sigemptyset(&blocked);
  ...
  if(!builtin_cmd(argv1)){
    ...
    /* put the child in a new process group whose group ID is idential to the child's PID.
       this ensures that there will be only one process, your shell, in the foreground
       process group. When you type ctrl-c the shell should catch the resulting SIGINT and
...
int builtin_cmd(char **argv) 
{
  ...
  // if the command is quit, exit with status 0
  if(!strcmp(cmd, "quit")) {
    exit(0);
  }
  
  // list the jobs from job list
...
void do_fg(int jid) 
{
  ...
  if(currentJob != NULL){
    ...
    // printf("Job [%d] (%d) stopped by signal 20", currentJob->jid, currentJob->pid);
  }
  else
    printf("%%%d: No such job\n", jid);
}
...
void waitfg(pid_t pid)
{
  // check if the process is fg or bg (fg and bg is saved as job objects)
  struct job_t *currentJob = getjobpid(jobs, pid);
  while(currentJob->state == FG && currentJob != NULL) {
    sigset_t empty;
    sigemptyset(&empty);
    sigsuspend(&empty);
  }
...
void sigchld_handler(int sig) 
{
  ...
  while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)
  {
    ...
    // check if exited
    if(WIFEXITED(status)){
      deletejob(jobs, pid);  // delete if normal exit
    }
    // check if stopped
